 ...
 *      Version: v2.2.1 (February 16, 2023)
 */
...
void main(void)
{
    ...
    unsigned short     length = 16;  // Length of the string buffer.
    char     rxstring[16] = {0x00};  // String buffer to store incoming messages from PC.
    ...
    // '#': LED COMMAND
            case '#':
                ...
                UCA0TXBUF = c;
                length = 16;
                memset(rxstring, 0x00, length);    // Clear the receive string container by populating it with NULL.
                while (i <= length -1)
                {
                    __bis_SR_register(LPM0_bits);  // Go to sleep while user inputs the next character. Program will resume execution at the next line when the use inputs a new character.
                    // User can press ENTER to finish inputting a value if their input contains less than 'length' characters.
                    // Alternatively, check for space to account for the trailing space (0x20) introduced by stinky batch echo commands.
                    if (UCA0RXBUF == '\r' || UCA0RXBUF == 0x20)
                    {
                        break;
                    }
                    else
                    {
                        rxstring[i] = UCA0RXBUF;       // Save the incoming character
                ...
                // The command should be AT LEAST four characters long, everything else is extra
                // FaLb (take 1 & 3), FcLd (5, 7), FeLf (9, 11), FgLh (13, 15)
                ...
                switch (i)
                {
                    case 4: // Case where the command is 4 characters long
                        SetBrightness(rxstring[1],rxstring[3]);
                        break;
                    case 8: // Case where the command is 8 characters long
                        SetBrightness(rxstring[1],rxstring[3]);
                        SetBrightness(rxstring[5],rxstring[7]);
                        break;
                    case 12: // Case where the command is 12 characters long
                        SetBrightness(rxstring[1],rxstring[3]);
                        SetBrightness(rxstring[5],rxstring[7]);
                        SetBrightness(rxstring[9],rxstring[11]);
                        break;
                    case 16: // Case where the command is 16 characters long
                        SetBrightness(rxstring[1],rxstring[3]);
                        SetBrightness(rxstring[5],rxstring[7]);
                        SetBrightness(rxstring[9],rxstring[11]);
                        SetBrightness(rxstring[13],rxstring[15]);
                        break;
                    default:
                        UARTsendMsg("  [ERROR]: LED command must be 4, 8, 12, or 16 characters long. Current length is ");
                        // Show the reason for the error.
                        itoa(i, int_string, 10);
                        UARTsendMsg(int_string);
                        UARTsendMsg(".\r\n\n");
                        printf("Received string in hex: %x", rxstring);
                        _ERROR = 1;
                        break;
                }
                if (!_ERROR){
